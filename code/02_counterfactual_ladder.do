...
*              USMCA stable at h2avg). The clean policy-relevant signal.
...
preserve
    di as text "      S3: rate_all_pref x con_val_mo (+ non-USMCA prefs)"
...
preserve
    di as text "      S2 by country (rate_h2avg x con_val_mo)"
    compute_tier, ratevar(rate_h2avg) weightvar(con_val_mo) ///
        outfile(`cty_s2') outvar(s2) byvar(partner_group) percent
restore

preserve
    di as text "      S3 by country (rate_all_pref x con_val_mo)"
    compute_tier, ratevar(rate_all_pref) weightvar(con_val_mo) ///
