...
**Figure naming convention:** every `figure_*.png` is exported in two versions — `figure_X.png` (no titles/subtitles, default for slides) and `figure_X_titled.png` (with titles/subtitles, for the paper draft). The dual-export is handled by the `export_dual_titles` helper in `programs.do` (for non-by-graphs) or an inline `foreach v in titled clean { ... }` loop (for by-graphs and `heatplot` where post-hoc title clearing isn't reliable).
...
- `export_dual_titles, base() grname() [width()] [notitled]` — wrapper around `graph display` to export the currently-active graph in two versions: `figure_X_titled.png` (current titled state) and `figure_X.png` (titles cleared via `graph display, title("") subtitle("")`). Used by every non-by graph in 03/03b/06; by-graphs use an inline foreach loop instead.
